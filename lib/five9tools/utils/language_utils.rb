...
module LanguageUtils
    ...
    def googlify(language_code_id)
      shortened_language_code_id = get_shortened_language_code(language_code_id)
      if GOOGLE_REPLACEMENT_LOCALES.keys.include?(shortened_language_code_id.to_sym) then return GOOGLE_REPLACEMENT_LOCALES[shortened_language_code_id.to_sym] else return shortened_language_code_id end
    end
